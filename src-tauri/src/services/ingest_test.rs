...
fn stores_content_once_under_its_sha256_path() {
    ...
    let second =
        import_original(&source, &temp.path().join("originals")).expect("duplicate import");
...
fn creates_distinct_inbox_records_that_share_a_duplicate_original() {
    ...
    let first_original =
        import_original(&source, &temp.path().join("originals")).expect("first import");
    let first = database
        .record_inbox_item("is-lm.pdf", &first_original, None)
        .expect("first inbox item");
    let second_original =
        import_original(&source, &temp.path().join("originals")).expect("duplicate import");
    let second = database
        .record_inbox_item("is-lm.pdf", &second_original, None)
